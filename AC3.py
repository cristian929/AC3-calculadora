...
class Calculadora (object):
    ...
    def calculadora(self, primeiro, segundo, operador):
        operacaoFabrica = OperacaoFabrica()
        operacao = operacaoFabrica.criar(operador)
        if(operacao == None):
            return 0
        else: 
            resultado = operacao.executar(primeiro, segundo)
            return resultado
...
class Operacao(metaclass=ABCMeta):
    ...
    @abstractmethod
    def executar(self, primeiro, segundo):
        pass

class Soma(Operacao):
    def executar(self, primeiro, segundo):
        resultado = valorprimeiro + valorsegundo
        return resultado
